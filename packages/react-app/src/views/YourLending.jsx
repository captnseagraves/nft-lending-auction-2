...
export default function YourLending({
  ...
  const [yourLendingAuctions, setLendingAuctions] = useState([]);
  ...
  useEffect(() => {
    const updateYourLendingAuctions = async () => {
      console.log("SEE ME SEE ME SEE ME yourLendingAuctions", yourLendingAuctions)
      const yourLendingAuctionsUpdate = [];
          ...
          if (loanAtIndex.lender == address) {
            try {
              ...
              try {
                ...
                yourLendingAuctionsUpdate.push({
      ...
      setLendingAuctions(yourLendingAuctionsUpdate);
    };
    updateYourLendingAuctions();
  }, [loanCreatedEvents, loanUnderwrittenEvents]);

  return (
    <div>
    <p>Your Lending Loan Auctions</p>   
    <div style={{ width: 640, margin: "auto", marginTop: 32, paddingBottom: 32 }}>
              <List
                bordered
                dataSource={yourLendingAuctions}
                renderItem={item => {
                    ...
                    <List.Item key={item.loanId + "_" + item.tokenAddress}>
                      ...
                      <div style={{ margin: 8 }}>
                        <Button
                          style={{ marginTop: 8 }}
                          onClick={async () => {
                            const result = tx(writeContracts.LendingAuction.seizeNFT(
                                item.loanId
                              ), update => {
